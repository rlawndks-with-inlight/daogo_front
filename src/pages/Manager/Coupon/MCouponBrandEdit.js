...
const MCouponBrandEdit = () => {
    ...
    useEffect(() => {
        async function fetchPost() {
            if (params.pk > 0) {
                const { data: response } = await axios.get(`/api/item?table=coupon_brand&pk=${params.pk}`);
    ...
    const CouponBrand = async () => {
        if ((!url && !content) || !$('.name').val()) {
            ...
            if (window.confirm('저장 하시겠습니까?')) {
                ...
                formData.append('table', 'coupon_brand');
    ...
    return (
        <>
            <Breadcrumb title={`쿠폰 브랜드 ${params.pk == 0 ? '추가' : '수정'}`} nickname={``} />
            <Card>
            ...
            <ButtonContainer>
                ...
                <AddButton onClick={CouponBrand}>{'저장'}</AddButton>
            </ButtonContainer>

        </>
    )
}
export default MCouponBrandEdit