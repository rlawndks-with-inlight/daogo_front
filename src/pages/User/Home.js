    ...
    return (
        <>
            <OneCard style={{ marginBottom: '12px', width: `${width ? width : ''}%` }} color={font_color} background={background ?? '#fff'} is_hover={true} onClick={() => navigate(link)}>
                ...
                <div style={{ fontSize: theme.size.font4, margin: `${width ? '0.15rem 1rem 0.15rem auto' : '0.15rem 50% 0.15rem auto'}`, display: 'flex', alignItems: 'center',fontWeight:'bold' }}><div>{commarNumber(content)}</div><div style={{ marginLeft: '8px' }}>{unit}</div></div>
