...
const Home = () => {
    ...
    useEffect(() => {
        ...
        let top_menu_list = [[{ title: "보유 STAR", content: "4000.00 STAR" }, { title: "총 발생 STAR", content: "8000.00 STAR" }],
        ...
        let bottom_menu_list = [[{ title: "출금 내역", link: "/" }, { title: "이체 하기", link: "/" }, { title: "ESGW POINT 구매", link: "/" }], [{ title: "계좌 등록", link: "/" }, { title: "문의 하기 Kakaotalk", link: "/" }, { title: "마이 페이지", link: "/" }]];
                                ...
                                <>
                                    <Row style={{ margin: '6px 0' }}>
                                        {item.map((itm, idx) => (
                                            <>
                                                <OneThirdCard>
                                                    <div style={{ fontSize: '10px', color: theme.color.font2 }}>{itm.title}</div>
                                ...
                                <>
                                    <Row style={{ margin: '6px 0' }}>
                                        {item.map((itm, idx) => (
                                            <>
                                                <OneSecondCard>
                                                    <div style={{ fontSize: '10px', color: theme.color.font2, margin: 'auto' }}>{itm.title}</div>
                                                </OneSecondCard>
                                            </>
                                        ))}
                                    </Row>
